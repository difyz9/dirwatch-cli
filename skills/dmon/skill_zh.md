...
默认配置是 `~/.config/dmon/dmon.yaml`。文件不存在时运行：
...
```bash
dmon retry EVENT_ID --reason '阶段: 简短错误'
```
...
直接运行不带子命令的 `dmon`。该模式输出 NDJSON 并自动确认，使用简单，但消费者在接收后失败可能造成事件丢失。Agent 的可靠处理应优先使用 `next`。
...
- 不要为了判断文件是否就绪而读取大文件内容；dmon 已通过 size 和 mtime 判断稳定性。
