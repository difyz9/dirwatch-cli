---
name: dmon
description: Install and use dmon to detect completed files in local, mounted, or NFS directories without reading file contents. Use when an agent must wait for newly completed files, process directory arrivals exactly once at the application level, acknowledge or retry file events, archive processed files, inspect delivery state, or configure recursive directory monitoring.
---
...
Use the default `~/.config/dmon/dmon.yaml`. If it does not exist, run:
...
```bash
dmon next --timeout 60s --lease 30m --max-files 1
```
...
```bash
dmon retry EVENT_ID --reason 'stage: concise error'
```
...
```bash
dmon next --timeout 60s --lease 30m \
...
Run `dmon` with no subcommand. This mode emits NDJSON and auto-confirms events; it is simpler but can lose an event if the consumer fails after output. Prefer `next` for reliable Agent work.
...
```bash
dmon queue list --status claimed
...
- Do not read large file contents merely to validate readiness; dmon already uses size and mtime stability.
